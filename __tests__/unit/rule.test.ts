...
const loggerService: LoggerService = new LoggerService({ maxCPU: 1, functionName: 'rule-902Test', nodeEnv: 'test' });

const ruleConfig: RuleConfig = {
  id: 'DEFAULT-902@1.0.0',
  tenantId: 'DEFAULT',
  cfg: '1.0.0',
  ...
  config: {
    ...
    bands: [
      ...
      },
      {
        ...
        lowerLimit: 2,
        upperLimit: 4,
        reason: 'The creditor has received two or three transactions to date',
      },
      {
        subRuleRef: '.03',
        lowerLimit: 4,
        reason: 'The creditor has received 4 or more transactions to date',
      },
    ],
  },
};
...
describe('Happy path', () => {
  test('Should respond with .01: The creditor has received one transaction to date', async () => {
    ...
    expect(res).toEqual(
      JSON.parse('{"id":"DEFAULT-902@1.0.0", "cfg":"1.0.0","subRuleRef":".01","reason":"The creditor has received one transaction to date", "tenantId":"DEFAULT"}'),
    );
  });

  test('Should respond with .02: The creditor has received two or three transactions to date', async () => {
    const mockQueryFn = jest.fn();
    ...
    expect(res).toEqual(
      JSON.parse(
        '{"id":"DEFAULT-902@1.0.0", "cfg":"1.0.0","subRuleRef":".02","reason":"The creditor has received two or three transactions to date", "tenantId":"DEFAULT"}',
      ),
    );
  });
  ...
  test('Should respond with .03: The creditor has received 4 or more transactions to date', async () => {
    const mockQueryFn = jest.fn();
    ...
    expect(res).toEqual(
      JSON.parse('{"id":"DEFAULT-902@1.0.0", "cfg":"1.0.0","subRuleRef":".03","reason":"The creditor has received 4 or more transactions to date", "tenantId":"DEFAULT"}'),
    );
  });
});
...
describe('Exit conditions', () => {
  test('Should respond with .x00: Incoming transaction is unsuccessful', async () => {
    ...
    expect(res).toEqual(
      JSON.parse('{"id":"DEFAULT-902@1.0.0", "cfg":"1.0.0","subRuleRef":".x00","reason":"Incoming transaction is unsuccessful", "tenantId":"DEFAULT"}'),
    );
  });
});
